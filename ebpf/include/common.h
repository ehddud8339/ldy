...
/* 전역 correlation ID
 * - 한 번의 write syscall(or 논리적인 I/O 요청)을 식별하는 64-bit 값
 * - sys_enter_write에서 모노토닉하게 증가시키며 생성
 */
typedef unsigned long long io_req_id_t;

// ===========================
// 설정 값 구조체
// ===========================

struct io_filter_conf {
  __u32 enabled;		  // 0 = 필터 끔, 1 = 켜짐
  __u32 use_tgid;		  // 1이면 tgid 기준 필터
  __u32 use_comm;		  // 1이면 comm 기준 필터

  __u32 tgid;			  // 추적할 tgid (== userspace PID)

  char comm[TASK_COMM_LEN]; // 추적할 프로세스 이름 (comm)
};
 ...
 * - req_id: 전역 correlation ID (sys_enter_write에서 생성)
 * - tgid/pid: 편의용(디버깅/추가 필터링)
 */
struct fs_key {
  io_req_id_t req_id; // 전역 correlation ID
  __u32		  tgid;   // 프로세스 ID
  __u32		  pid;	  // 스레드 ID
};
 ...
 * - 실제 correlation은 rq_ptr -> req_id 맵에서 수행
 */
struct rq_key {
 ...
 * - req_id: 이 이벤트가 속한 I/O 요청 (correlation ID), 없으면 0
 */
struct io_evt_hdr {
  ...
  io_req_id_t req_id;	// correlation ID (없으면 0)
};
 ...
 * - hdr.req_id와 key.req_id는 동일한 값을 갖도록 사용
 */
 ...
 *	 fs_key.req_id == 0 인 경우 "syscall과 직접 매칭되지 않음" 으로 해석 가능
 */
struct evt_ext4_write_begin {
  struct fs_key  key;	// 유저 문맥이면 유효, flusher면 req_id==0 일 수 있음
  struct file_id fid;
 ...
 *	 (필요하다면 나중에 별도 correlation 전략을 추가)
 */
 ...
 * - hdr.req_id 에는 rq_ptr -> req_id 매핑 결과를 채워 넣는다.
 */
struct evt_blk_rq {
  struct rq_key rq;		   // struct request * 포인터
  io_req_id_t	req_id;    // 이 request에 매핑된 correlation ID (없으면 0)
 ...
 * - hdr.req_id / data.req_id 모두 rq_ptr -> req_id 매핑 결과를 채운다.
 */
 ...
 * - hdr.req_id는 가능한 모든 계층에서 동일한 correlation ID를 유지하도록 채운다.
 */
struct io_evt {
