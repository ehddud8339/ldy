...
/* writev 직전에 찍는 더미 tracepoint */
__attribute__((noinline))
void fuseflow_user_sendmsg_enter(struct fuse_session *se,
                                 struct fuse_chan *ch,
                                 struct iovec *iov,
                                 int count)
{
...
/* writev 직후에 찍는 더미 tracepoint (여기에서만 BPF 이벤트 전송) */
__attribute__((noinline))
void fuseflow_user_sendmsg_exit(struct fuse_session *se,
                                struct fuse_chan *ch,
                                struct iovec *iov,
                                int count,
                                ssize_t res,
			 ...
			 struct iovec *iov, int count)
{
  ...
  // LDY
  fuseflow_user_sendmsg_enter(se, ch, iov, count);
  ssize_t res = writev(ch ? ch->fd : se->fd, iov, count);
  fuseflow_user_sendmsg_exit(se, ch, iov, count, res, 0);
  // LDY
  /*
...
// LDY
/* read 직전에 찍는 더미 tracepoint */
__attribute__((noinline))
void fuseflow_user_dev_read_enter(struct fuse_session *se,
                                  struct fuse_buf *buf,
                                  struct fuse_chan *ch)
{
    (void)se;
...
/* read 직후에 찍는 더미 tracepoint (여기서 uprobe로 이벤트 보냄) */
__attribute__((noinline))
void fuseflow_user_dev_read_exit(struct fuse_session *se,
                                 struct fuse_buf *buf,
                                 struct fuse_chan *ch,
                                 ssize_t res,
                                 int err)
...
restart:
  // LDY 
  fuseflow_user_dev_read_enter(se, buf, ch);
  res = read(ch ? ch->fd : se->fd, buf->mem, se->bufsize);
  GET_TIMESTAMPS(6)
  fuseflow_user_dev_read_exit(se, buf, ch, res, 0);
  // LDY 
  /*
	res = read(ch ? ch->fd : se->fd, buf->mem, se->bufsize);
...
int fuse_req_getgroups(fuse_req_t req, int size, gid_t list[])
{
	...
	printf(path, "/proc/%lu/task/%lu/status", pid, pid);
